...
class Experts(nn.Module):
    def __init__(self,
        ...
        if self.use_mpo:
            if dim == 768:
                mpo_input_shape_fc, mpo_output_shape_fc, truncate_num_fc = [3,4,4,4,4],[4,4,8,6,4], config.linear_trunc # 768*3072
                mpo_input_shape_proj, mpo_output_shape_proj, truncate_num_proj = [4,4,8,6,4], [3,4,4,4,4], config.linear_trunc # 3072*768
            elif config.n_embd == 1024:
                mpo_input_shape_fc, mpo_output_shape_fc, truncate_num_fc = [4,4,4,4,4], [4,4,8,8,4], config.linear_trunc # 1024*4096
                mpo_input_shape_proj, mpo_output_shape_proj, truncate_num_proj = [4,4,8,8,4], [4,4,4,4,4], config.linear_trunc # 4096*1024
            else:
    ...
    def forward(self, x):
        if self.use_mpo:
            ...
            ######## 上面这个需要调换
            hidden = torch.einsum('...nd,...dh->...nh', x, w1_rebuild) + self.b1
            hidden = self.act(hidden)
            out    = torch.einsum('...nh,...hd->...nd', hidden, w2_rebuild) + self.b2
        else:
...
class MoE(nn.Module):
    ...
    def from_pretrained_mpo(self, expert_new=None, use_mpo=False):
        if expert_new and not use_mpo:
            ...
            self.experts.w1.data = expert_new.wi.weight.data.clone().repeat(*w1_shape)
            self.experts.w2.data = expert_new.wo.weight.data.clone().repeat(*w2_shape)
